...
class LighthouseClient implements LighthouseClientInterface {
  ...
  public function getToken() {
    ...
    $endpoint_path = $this->getApiPaths()['get_token'] ?? '';
    $endpoint_full_path = $configuration['base_path'] . ':' . $configuration['port'] . $configuration['subpath'] . $endpoint_path;
  ...
  public function search($text, $filters = [], $sort_by = [], $offset = 0, $limit = 10, $params = []) {
    ...
    $configuration = $this->getConfiguration();
    $endpoint_path = $this->getApiPaths()['search'] ?? '';
    $endpoint_full_path = $configuration['base_path'] . ':' . $configuration['port'] . $configuration['subpath'] . $endpoint_path;
