...
class SearchGridBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function __construct(
    ...
    $this->gridId = $grid_id ? ++$grid_id : 1;
    $this->buildDataLayerDefaults();
  }
  ...
  protected function buildDataLayerDefaults() {
    ...
    $this->dataLayerDefaults = [
      'cardGridID' => $this->gridId,
      'cardGridName' => $blockConfig['title'],
    ];
  }

  /**
   * Builds an array of dataLayer attributes for search input.
   *
   * @return array
   *   DataLayer attributes.
   */
  protected function getDataLayerDefaults() {
    if (empty($this->dataLayerDefaults)) {
      $this->buildDataLayerDefaults();
    }
    return $this->dataLayerDefaults;
  }
